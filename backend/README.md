...
- one-time bootstrap API для первого запуска системы без `seed.sql`
- bootstrap с обязательным parent-administrator и BCrypt master password
- admin API для добавления и удаления детей, закрытое bearer token + master password
...
- `setup/status` на чистой БД
- `setup/bootstrap` и запрет повторной инициализации
- admin unlock/create/remove child flow
- `login` happy path
- повторное использование invite одним и тем же пользователем
- `login` создаёт устройство для новой платформы
...
```bash
curl -X POST http://localhost:8081/api/setup/bootstrap \
  -H 'Content-Type: application/json' \
  -d '{
      "masterPassword":"super-secret-pass",
      "familyName":"Family One",
      "members":[
      {"displayName":"Parent","role":"parent","isAdmin":true},
      {"displayName":"Child","role":"child"}
    ]
  }'
```

Login по invite-коду:
...
```bash
curl "http://localhost:8081/api/messages/sync?since_id=0" \
  -H "Authorization: Bearer <token>"
```
...
```bash
open http://localhost:8081/api-docs/swagger
```
